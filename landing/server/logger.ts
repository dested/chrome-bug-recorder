...
const ASSET_EXT =
  /\.(js|mjs|ts|tsx|jsx|css|map|png|jpe?g|gif|svg|ico|webp|avif|woff2?|ttf|eot|wasm|mp4|webm)$/i
...
export function startupBanner(opts: { port: number; isProd: boolean; routes: string[] }) {
  const mode = opts.isProd ? c.yellow('production') : c.green('development')
  const url = c.cyan(`http://localhost:${opts.port}`)
  console.log()
  console.log(`  ${c.bold(c.magenta('gripe-landing'))} ${c.dim('· Express + React Router 7 SSR')}`)
